...
When API access is needed (listing projects, checking branches, etc.):
...
3. Use it in API calls within the same command chain:
   ```bash
   ssh <devbox> 'TOKEN=$(grep "172.16.20.50" ~/.git-credentials | sed "s/.*:\(glpat-[^@]*\)@.*/\1/") && curl -s "http://172.16.20.50:8929/api/v4/projects?private_token=$TOKEN&search=<query>"'
   ```
